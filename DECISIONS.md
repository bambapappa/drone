...
- **Känd approximation:** spatio-temporal-porten använder råa bildrutepixlar (ingen GMC-stabilisering persisterad i Fas 0) — en rimlighetsfilter, inte en rörelsemodell. Full stabilisering är ett P2/trajectory-arbete när `stab_offset` persisteras; porten förblir bindande för korta gap och appearance+temporal-uteslutning gör det verkliga identitetsarbetet.
...
- **Validering:** 368 tester gröna (`test_irrational.py`, `test_hazard.py` nya, `test_annotations.py`/`test_review_api.py` utökade med faromarkör-täckning), ruff lint/format rent, hela flödet manuellt verifierat mot en körande server (seed-körning + curl: IRRATIONELLT-händelse med `evidence.summary`, faromarkör placerad → MOT FARA omräknad direkt i nästa `/events`-anrop, faromarkör borttagen → återgår till motorns egen härledning). Determinismtester: samma indata två gånger → bit-identiska IRRATIONELLT-händelser; samma faromarkörsposition två gånger (inklusive efter att ha flyttat den bort och tillbaka) → bit-identiska MOT FARA-händelser.
