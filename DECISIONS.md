...
### B22. Offline-läge (efteranalys) — detektion på varje ruta + scrubbar spelare
- **Val:** ett separat icke-realtidsläge (`app/vision/offline.py` + `scripts/analyze_offline.py`) som kör detektionen på *varje* ruta och skriver en bunt (`meta.json`, `frames.jsonl`, `events.json`, `state.json`), spelad upp på `/player` med spola fram/tillbaka, ruta-för-ruta-stegning och klickbar händelse­tidslinje.
- **Varför inte återanvända live-pipelinen rakt av:** live-pipelinen offrar detektionskvalitet för låg fördröjning (detektion på bakgrundstråd några Hz + flödesframmatning av boxar, B3). För efteranalys finns ingen latensbudget — vi vill ha bästa möjliga markering, alltså detektion på varje ruta utan flödesgissning. Offline återanvänder *komponenterna* (detector/registry/behavior/situation/global motion) men kör enkeltrådat och synkront; ingen tvåtrådsakrobatik behövs.
- **Samma metadataschema som live** → spelaren återanvänder exakt samma ritkod. Ritfunktionerna lyftes ut ur `app.js` till `static/overlay.js` (delad av live + spelare), vilket samtidigt minskar synk-skulden mellan renderarna (jfr logg 2026-06-12).
- **Bilder dupliceras inte** i bunten — spelaren strömmar originalfilmen (FileResponse med Range → native sökning) och lägger annoteringar ovanpå, synkat på videotid. `t = frame_idx / fps` ⇒ deterministisk, ej väggklocka.
- **Syfte (beställarmål):** granska vad modellen *missade* (rapporterat 50–90 % missade på riktig film) och om besluten var rätt; A/B:a modell/upplösning ruta-för-ruta. Markering av facitrutor för mätning/finjustering är nästa steg — bunten är utgångsläget att rätta mot.
- **Begränsning:** spelarens overlay antar obeskuren film; med `ANALYSIS_ROI` ligger markeringarna i den beskurna vyns koordinater. Bakgrundskörningen (`POST /api/analyze`) är en enkel subprocess utan kö — en körning per film i taget.
...
- 2026-06-16: Offline-läge / efteranalys (B22): detektion på varje ruta → bunt, scrubbar spelare på `/player` (spola fram/tillbaka, ruta-för-ruta, händelsetidslinje). Ritkoden delad via `static/overlay.js`. 68 tester gröna (5 nya, ML-fria via stub-detektor); end-to-end-rökverifierad (bunt skapad + API-routrar + path-traversal-skydd). Avsett att diagnosticera de 50–90 % missade detektionerna och underbygga beslut om träning.
