...
- **Begränsning:** integrationsverifiering live uteblev denna session — uvicorn-servern startade inte i sandlådan (miljö-/buffringsproblem, ej kodfel). Funktionen täcks av 6 enhetstester + offline-körning mot exakt produktionskodvägen (`detect_pip_frame`/`PipAutoDetector`) på allt riktigt material. Bör bekräftas live på GPU. Split-fall maskeras f.n. (ej beskärs) — enkel men slösar halva detektorupplösningen; beskärning är en framtida förbättring.
